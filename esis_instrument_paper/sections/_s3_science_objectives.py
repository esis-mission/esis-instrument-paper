...
This allows us to resolve complicated morphologies of compact \TR\ reconnection events (as was done with
...
number of observations, or a consequence of the morphology of the event being difficult to ascertain from
slit spectrograph data.
...
Since the solar wind is not accelerated to an appreciable fraction of the Alfv\'en wave velocity at altitudes
...
\SI{18}{\kilo\meter\per\second} in the \OVion\ (\SI{0.25}{\mega\kelvin}) line.
...
The torsional motion is observed as Doppler shifts when viewed from the side.
The oscillation period is long enough to be well resolved but short enough to see $\sim$\num{10} cycles in a single
rocket flight.
...
\end{inparaenum}
The resulting non-thermal widths for localized sources are significantly higher than the
$\sim$\SI{18}{\kilo\meter\per\second} lower limit derived above.
...
we expect that with the \numChannelsWords\ \ESIS\ projections it is possible to separate the \OVion\ emission from
that of \MgXion.
From the important temporal, spatial, and velocity scales referenced in Sections~\ref{subsec:MagneticReconnectionEvents}
